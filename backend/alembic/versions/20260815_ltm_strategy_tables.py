...
Revises: 20260812_str005_strategy_execution
...
def upgrade() -> None:
    ...
    if "ltm_book_state" not in existing_tables:
        op.create_table(
            "ltm_book_state",
    ...
    if "strategy_scan_latest" not in existing_tables:
        op.create_table(
    ...
    if "strategy_scan_runs" not in existing_tables:
        op.create_table(
            "strategy_scan_runs",
...
def downgrade() -> None:
    ...
    inspector = sa.inspect(conn)
    existing_tables = set(inspector.get_table_names())
    if "strategy_scan_runs" in existing_tables:
        op.drop_table("strategy_scan_runs")
    if "strategy_scan_latest" in existing_tables:
        op.drop_table("strategy_scan_latest")
    if "ltm_book_state" in existing_tables:
