...
class PaperPosition(Base):
    ...
    total_buy_charges: Mapped[Decimal] = mapped_column(Numeric(18, 2), default=Decimal("0.00"), server_default=text("0.00"))
    break_even_price: Mapped[Decimal | None] = mapped_column(Numeric(18, 8), nullable=True)
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), default=datetime.utcnow, index=True)
    updated_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), default=datetime.utcnow, onupdate=datetime.utcnow, index=True)
...
class PaperTradeHistory(Base):
    ...
    total_charges: Mapped[Decimal | None] = mapped_column(Numeric(18, 2), nullable=True, default=Decimal("0.00"))
