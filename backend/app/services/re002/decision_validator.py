"""Decision Object completeness and fail-closed rules (FR-004/024/025/026)."""
...
from ...schemas.re002 import Re002DecisionObject, RecommendationState


ALLOWED_STATES = frozenset({"BUY", "WATCH", "REJECT"})


def validate_decision_object(obj: Re002DecisionObject | dict[str, Any]) -> tuple[bool, list[str]]:
    errors: list[str] = []
    if isinstance(obj, Re002DecisionObject):
        data = obj.model_dump()
    else:
        data = obj
    ...
    if state == "BUY":
        ...
        if "weak_relative_strength" in (reasons or []):
            errors.append("buy_with_weak_rs")
        if "missing_relative_strength" in (reasons or []):
            errors.append("buy_with_missing_rs")
...
def coerce_state_for_reasons(
    ...
    if "missing_market_context" in reason_codes:
        return "REJECT"
    if "weak_relative_strength" in reason_codes:
        return "REJECT"
    if "missing_relative_strength" in reason_codes:
        return "REJECT"
    if "failed_bull_stock_filter" in reason_codes:
        return "REJECT"
    if "insufficient_history" in reason_codes:
        return "REJECT"
