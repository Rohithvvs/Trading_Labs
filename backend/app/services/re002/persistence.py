"""Persist and query RE-002 Decision Objects in multi-engine table."""
...
def persist_decision(
    ...
    *,
    ...
    if not settings.re002_persist_decisions:
        return None
    try:
        ...
        kwargs: dict[str, Any] = dict(
            ...
            if isinstance(decision.risk_profile, dict)
            else {"value": decision.risk_profile},
            portfolio_decision=(
        ...
        if hasattr(RecommendationEngineDecision, "experiment_id"):
            kwargs["experiment_id"] = decision.experiment_id

        row = RecommendationEngineDecision(**kwargs)
        db.add(row)
        db.commit()
        db.refresh(row)
        return row
        ...
        if existing is not None:
            try:
                ...
                pass
            logger.info(
                "RE-002 persist idempotent | recommendation_id=%s | symbol=%s | scan_run_id=%s",
            ...
            return existing
        logger.warning(
            "RE-002 persist IntegrityError without existing row | recommendation_id=%s",
            decision.recommendation_id,
            exc_info=True,
        )
        return None
        ...
        logger.warning(
            "RE-002 persist failed | recommendation_id=%s | symbol=%s | scan_run_id=%s | err=%s",
            decision.recommendation_id,
            decision.symbol,
            decision.scan_run_id,
            exc,
...
def list_decisions_for_scan(
    db: Session, scan_run_id: str, *, engine_id: str = "RE-002"
) -> list[RecommendationEngineDecision]:
    return (
...
def get_latest_for_symbol(
    db: Session, symbol: str, *, engine_id: str = "RE-002"
) -> RecommendationEngineDecision | None:
...
def row_to_decision_dict(row: RecommendationEngineDecision) -> dict[str, Any]:
    ...
    if hasattr(row, "experiment_id"):
        data["experiment_id"] = getattr(row, "experiment_id", None)
    return data
