...
@pytest.mark.anyio
async def test_lab_universe_reuses_injected_market_regime():
    """run_independent_lab_universe must not re-evaluate when market_regime is provided."""
    ...
    with patch(
        "app.services.scan_market_context.get_or_build_market_regime",
        new_callable=AsyncMock,
    ) as mock_build:
        # Empty universe returns early before engine checks — still must not build regime.
        summary = await run_independent_lab_universe(
            symbols=[],
            market_regime=regime,
        )
    assert summary["input_universe"] == 0
    mock_build.assert_not_called()


@pytest.mark.anyio
async def test_lab_universe_does_not_rebuild_when_context_set():
    from app.services.independent_lab_universe import run_independent_lab_universe
    ...
    smc.set_scan_market_regime(regime)
    ...
    with (
        ...
        patch(
            "app.services.independent_lab_universe._load_missing_candles",
            new_callable=AsyncMock,
            side_effect=lambda symbols, existing, **kw: existing,
        ),
        patch(
        ...
        # One symbol, no candles → workers no-op quickly
        summary = await run_independent_lab_universe(
            symbols=["RELIANCE-EQ"],
            market_regime=None,  # should pull from context
            scan_run_id="lab-ctx",
        )

    mock_build.assert_not_called()
    assert summary["input_universe"] == 1
