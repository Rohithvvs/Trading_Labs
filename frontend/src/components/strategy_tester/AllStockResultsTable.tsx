  ...
  const sortedRows = React.useMemo(() => {
    ...
    return [...filteredResults].sort((a, b) => {
      ...
      if (sortColumn === "entry_price") {
        const pA = a.entry_price ?? a.candidate_entry_price ?? a.close ?? 0;
        const pB = b.entry_price ?? b.candidate_entry_price ?? b.close ?? 0;
        return sortDirection === "asc" ? pA - pB : pB - pA;
      }
      if (sortColumn === "exit_price") {
        const pA = a.exit_price ?? a.close ?? 0;
        const pB = b.exit_price ?? b.close ?? 0;
        return sortDirection === "asc" ? pA - pB : pB - pA;
      }
      if (sortColumn === "rr") {
        const rrA = a.rr ?? 0;
        const rrB = b.rr ?? 0;
        return sortDirection === "asc" ? rrA - rrB : rrB - rrA;
      }
      return 0;
    });
        ...
        <table className="st-main-table" data-testid="table-strategy-results">
          <thead>
            <tr>
              ...
              {isColVisible("evaluation_date") && <th className="sortable" onClick={() => onSortChange("evaluation_date")}>{variant === "scanner" ? "ScanData" : "Scan Date"}</th>}
                  ...
                  >
                    ...
                    {isColVisible("entry_price") && (
                      <td style={{ textAlign: "right", fontVariantNumeric: "tabular-nums" }}>
                        {formatINR(row.entry_price ?? row.candidate_entry_price ?? row.close)}
                      </td>
                    )}
                    ...
                    {isColVisible("rr") && (
                      <td
                        style={{
                          ...
                          color: (row.rr ?? 0) >= 2 ? "#4ade80" : "#94a3b8",
                          fontWeight: (row.rr ?? 0) >= 2 ? 600 : 400,
                        }}
                        data-testid={`stock-rr-${row.symbol}`}
                      >
                        {formatRR(row.rr)}
                      </td>
                    )}
